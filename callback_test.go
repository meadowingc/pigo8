...
func TestCallbackRegistration(t *testing.T) {
	...
	config := network.ParseNetworkArgs()
	config.Role = network.RoleServer // Use server role for testing
	config.Address = "127.0.0.1"
	config.Port = 12345 // Use a test port

	if err := network.InitNetwork(config); err != nil {
	...
	network.SetOnGameStateCallback(func(playerID string, data []byte) {
	...
	network.SetOnPlayerInputCallback(func(playerID string, data []byte) {
