...
func (g *Game) Init() {
	...
	g.canvas = make([][]int, pigo8.ScreenHeight)
	for i := range g.canvas {
		g.canvas[i] = make([]int, pigo8.ScreenWidth)
...
func (g *Game) Update() {
	...
	if drawing || erasing {
		...
		if mouseY < pigo8.ScreenHeight-30 {
...
func (g *Game) Draw() {
	...
	// Draw info text
	pigo8.Print(fmt.Sprintf("mouse: %d,%d", mouseX, mouseY), 2, pigo8.ScreenHeight-10, 7)
	pigo8.Print(fmt.Sprintf("brush: %d", g.brushSize), 70, pigo8.ScreenHeight-10, 7)
